    ...
    <div className="container mx-auto px-4 py-8 max-w-5xl">
       <div className="flex justify-between items-center mb-8">
        <div>
           ...
           <p className="text-muted-foreground">Manage your scheduled lessons and learning history.</p>
        </div>
        <Link href="/professionals">
            <Button>
                <PlusIcon className="w-4 h-4 mr-2" /> Book New Session
            </Button>
        </Link>
      </div>
          ...
          {upcomingSessions.length > 0 ? (
            <div className="grid gap-4">
             ...
             <div className="bg-card border rounded-lg p-8 text-center">
                ...
                <p className="text-muted-foreground mb-4">You don't have any scheduled sessions coming up.</p>
                <Link href="/professionals">
                    <Button variant="outline">Find a Professional</Button>
                </Link>
             </div>
          )}
        </section>
...
function SessionCard({ booking, isUpcoming }: { booking: Booking, isUpcoming: boolean }) {
    ...
    return (
        <div className="bg-card rounded-lg border shadow-sm p-5 flex flex-col md:flex-row gap-6 items-start md:items-center">
            {/* Date/Time Column */}
            <div className="flex flex-col items-center justify-center min-w-25 bg-secondary/50 rounded-lg p-3 text-center border">
            ...
            <div className="flex-1 space-y-3">
                ...
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary overflow-hidden">
                        {booking.professional.user.image ? (
                            <img src={booking.professional.user.image} alt={booking.professional.user.name} className="w-full h-full object-cover" />
                        ) : (
                            <UserIcon className="w-5 h-5" />
                        )}
                    </div>
                    <div>
                        <div className="font-semibold">{booking.professional.user.name}</div>
                        <div className="text-xs text-muted-foreground flex items-center gap-1">
                             <GraduationCap className="w-3 h-3" /> {booking.professional.headline || "Language Professional"}
                        </div>
                    </div>
                </div>
            </div>
