...
import { startBookingCheckout, getAvailableSlots } from "@/app/actions/booking";
...
interface BookingFormProps {
  professional: any; // Server-fetched professional, includes Stripe fields
}
      ...
      <div className="space-y-4">
        ...
        <div className="pt-4 border-t">
          ...
          <Button
            ...
            onClick={handleBook}
            disabled={loading || !time || !professional.stripeChargesEnabled || !professional.stripePayoutsEnabled || !professional.stripeAccountId}
          >
            {loading ? <Loader2 className="animate-spin w-4 h-4 mr-2" /> : null}
            Book Session
          </Button>
          {(!professional.stripeChargesEnabled ||
            !professional.stripePayoutsEnabled ||
            !professional.stripeAccountId) && (
            <p className="text-xs text-center text-amber-600 dark:text-amber-400 mt-2">
              This professional is not currently accepting paid bookings. Stripe
              payouts are not fully set up.
            </p>
          )}
          <p className="text-xs text-center text-muted-foreground mt-1">
            You will be redirected to Stripe to complete payment. Your booking
            is only created after successful payment.
          </p>
        </div>
      </div>
