...
export function UserRow({ user, selfId }: { user: UserWithRole, selfId: string }) {
    ...
    function handleRevokeSessions(userId: string) {
        authClient.admin.revokeUserSessions({ userId }, {
            onError: (err) => {
                toast.error(err.error.message || "Failed to revock user sessions")
            },
            onSuccess: () => {
                toast.success("User sessions revocked")
                router.refresh()
            }
        })
    }
            ...
            <TableCell>
                <div>
                    ...
                    <div className="flex items-center gap-2 not-empty:mt-2">
                        {user.banned && <Badge variant="destructive">Baned</Badge>}
                        {!user.emailVerified && <Badge variant={"destructive"}>Unverified</Badge>}
                        ...
                        <AlertDialogContent>
                            <AlertDialogHeader>
                                <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>Are you sure you want to delete this user? This action can't be undone</AlertDialogDescription>
                                </AlertDialogHeader>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                                <AlertDialogCancel>Cancel</AlertDialogCancel>
                                <AlertDialogAction onClick={() => handleRemoveUser(user.id)} className="bg-destructive text-shadow-destructive hover:bg-destructive/90">
                                    Delete
                                </AlertDialogAction>
                            </AlertDialogFooter>
                        </AlertDialogContent>
